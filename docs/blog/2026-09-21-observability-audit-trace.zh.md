...
> **语言 / Language**：中文 ｜ 系列第十一章（[目录](./README.md)）｜ 上一章：[前端与流式交互](./2026-09-21-frontend-streaming-ux.zh.md) ｜ 下一章：[部署交付实录](./2026-09-21-deploy-delivery-hardening.zh.md)
>
...
可观测性最贵的一课来自延迟攻坚期（[第八章](./2026-09-16-agent-latency-stability-tuning.zh.md)的完整叙事）：
...
修复后每次模型调用都有出站行：`LLM出站 scene=回答生成 model=siliconflow-small durMs=7405 ok=true attempt=1/2`。而且**流式路径后来又漏了一次**——同步路径有出站日志，流式不经 `FailoverExecutor`，是零日志的（[第三章](./2026-09-21-model-gateway-resilience.zh.md)的补漏记录）。纪律因此定型一句话：**"无日志空白"从此不可能再发生——空白即线索**。观测覆盖不是一次性的，新路径（流式、旁路、批处理）每次都要重新回答"这条路径上日志空白意味着什么"。
...
- **业务前置校验**："confirm 放行前经 HitlBusinessGate 对账订单真实状态……不满足则不决议（工单保持 PENDING），返回 CONFLICT(409)"——[第七章](./2026-09-21-business-tools-workflow-dag.zh.md)的 mock↔DB 对账事故就是这条链上炸出来的。
...
![可观测三流：同一步骤产出分叉为实时进度、审计落库、指标快照三条正交信道](assets/2026-09-21-observability-three-channels.png)
...
5. **密钥启动校验类不存在**——与 DEPLOY.md"已知缺口"对账；双源提示词人工同步登记在第二章与[第十二章](./2026-09-21-deploy-delivery-hardening.zh.md)的已知边界。
...
> 相关阅读：[系列目录](./README.md) · [上一章：前端与流式交互](./2026-09-21-frontend-streaming-ux.zh.md) · [下一章：部署交付实录](./2026-09-21-deploy-delivery-hardening.zh.md) · [第三章·流式出站日志补漏](./2026-09-21-model-gateway-resilience.zh.md)
