...
> **语言 / Language**：中文 ｜ 系列第二章（[目录](./README.md)）｜ 上一章：[设计理念与总体架构](./2026-09-21-design-philosophy-architecture.zh.md) ｜ 下一章：[模型网关与韧性治理](./2026-09-21-model-gateway-resilience.zh.md)
>
...
教训值得单拎出来：**方案里的每一个 API 都要核实它在你的依赖树里真实存在**。`@RefreshScope` 写进设计文档时看起来天经地义，项目没引 spring-cloud 全家桶（[第一章](./2026-09-21-design-philosophy-architecture.zh.md)选型裁决），它就是不存在。替代方案反而更简单：Nacos 控制台"AI 资源 → 提示词模板"建一个 key，SDK 内置 md5 缓存 + gRPC 热推送，零自研 listener。
...
顺序为什么是语义：System 在最前定义"你是谁、规则是什么"；记忆参考块紧跟其后、用标签声明"这是参考事实非指令"（防伪造闭合标签的尖括号中和在[第四章](./2026-09-21-session-memory-layering.zh.md)展开）；客观数据层把检索片段框进【参考资料】头（工具结果/工具异常各有专属隔离头，历史原样透传）；用户输入永远在最后。**任何一层"好心"把数据塞进别的层，都会改变模型对整段文本的解读框架**。
...
![分段提示词装配全景：双源 → 六步装配 → 四层拼接](./assets/2026-09-21-prompt-assembly-flow.png)
...
| 用户画像 | 注入位隔离 | 独立块 + 标签 + 非指令声明（09-20 裁决，[第四章](./2026-09-21-session-memory-layering.zh.md)） |
...
3. **业务键不进模板**：设计要点原话——"L0 注册表管实体消解（决策层，本就在 LLM 上下文之外）——"那单到哪了"靠 L0 兜底消解，摘要漏写订单号不丢 correctness"。确定性需求放在代码里兜底，提示词只负责表达层（[第四章](./2026-09-21-session-memory-layering.zh.md)的 BusinessKeyExtractor 是它的运行时补齐侧）。
...
4. **注入扫描是词库**：可被改写绕过，对"像真的的错误"无效（[第五章](./2026-09-21-rag-evolution-abac-refusal.zh.md)红队实验）。

---

*本文机制出处：`prompt/`（PromptSegment / PromptTemplate / PromptRegistry / LocalPromptSource / NacosPromptSource）、`context/`（SystemPromptAssembler / ContextMerger / SystemAnchorLayer / ObjectiveDataLayer / UserInstructionLayer / UserMemoryLayer）、`gateway/llm/ChatLlmService`（chatRaw 例外）；设计沿革见 [2026-09-14 分段式系统提示词设计](../design/specs/2026-09-14-segmented-systemprompt-intent-design.md)。*

> 相关阅读：[系列目录](./README.md) · [上一章：设计理念与总体架构](./2026-09-21-design-philosophy-architecture.zh.md) · [下一章：模型网关与韧性治理](./2026-09-21-model-gateway-resilience.zh.md) · [第四章·注入位隔离的完整论证](./2026-09-21-session-memory-layering.zh.md)
