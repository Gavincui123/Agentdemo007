...
![RAG 检索漏斗七级数字：宽召回 24+24 → 权限过滤 → 域窄化 → 粗滤 0.2 → 条件重排 top-10 → 置信度终闸 → 注入扫描](https://raw.githubusercontent.com/Gavincui123/Agentdemo007/master/docs/blog/assets/2026-09-21-rag-funnel-numbers.png)
...
rt-001 实测记录里最有价值的观察：毒片与查询语义高度相关、**cosine 轻松过 0.4 终闸（污染真实参与了召回）**，是注入扫描的词库命中把它拦下的。所以红队结论不是"防线全防住了"：**扫描器防"注入"不防"像真的的错误"**——没有注入词面的假政策，会自信满满地走进回答。缓解是系统性的：冲突仲裁指令（历史片段仅作背景、不得作为现行答案；现行资料互斥时不得擅自裁决，如实说明存在不同口径）、citations 前端溯源、语料准入治理。完整实验与防线盲区标注见[红队演示手册](https://github.com/Gavincui123/Agentdemo007/blob/master/docs/reports/rag-redteam-conflict-demo.md)。
...
*本文数字与机制出处：`capability/rag/`（HybridRetriever / RetrievalValidator / Bm25Reranker / LuceneBm25IndexService）、`capability/kb/`（KbCatalogService / KbLevel）、`capability/refusal/`（RefusalGateStep）；交付报告与缺陷清单见 [2026-09-19 拒答+录入报告](https://github.com/Gavincui123/Agentdemo007/blob/master/docs/reports/2026-09-19-refusal-kb-ingest-report.md)。*

> 相关阅读：[系列目录](https://blog.csdn.net/qq_24993561/article/details/166257230) · [语料清洗切分入库教程](https://github.com/Gavincui123/Agentdemo007/blob/master/docs/guides/rag-corpus-ingestion-tutorial.md) · [下一章：决策层、持久化与闸门硬化（含真 RAG 收尾与毒性实验）](https://blog.csdn.net/qq_24993561/article/details/166257730)
