...
> **语言 / Language**：中文 ｜ [English](./2026-09-16-agent-latency-stability-tuning.en.md)
>
> **项目**：Agentdemo007 —— 小哲电商智能客服 Agent
...
| Embedding/Reranker HTTP | connect 20s / read 30s | — | 裸 `new RestTemplate()` 无超时是无限挂起根源 |
...
1. **HTTP 层**：`SimpleClientHttpRequestFactory` 显式设 connect 20s / read 30s；
...
flowchart TD
    ...
    E -->|"闭合"| EF["SiliconFlow 嵌入<br/>HTTP connect 20s / read 30s"]
    E -->|"开路：秒级降级"| SO["稀疏 BM25-only<br/>本地基线 · 永远可用"]
    EF -->|"超时/失败 → 单次即熔断"| SO
    EF -->|"成功"| HY["稠密 + 稀疏混合召回"]
    HY --> R{"② Reranker 断路器<br/>包在每个 API provider 内侧"}
    SO --> R
    R -->|"闭合"| RF["SiliconFlow 重排<br/>HTTP connect 20s / read 30s"]
...
候选集只保留模型可选项（CHIT_CHAT / REASONING / LONG_CONTEXT / STRUCTURED_EXTRACTION / OTHER），解析层再做一道防线：模型即便输出了 `TRANSFER_TO_HUMAN` 也按 UNKNOWN 处理，回退规则层。

提示词按可迁移的结构重写，五个部分缺一不可：
...
2. **改写器划红线**：`必须保留用户原问题原意（不可丢弃/篡改），只补充必要上下文使指代可消解；严禁替用户下业务结论`——改写器的职责是消解指代，不是替用户做意图归属。
...
| 位置 | 角色 |
|---|---|
| `SystemAnchorLayer.DEFAULT_SYSTEM_PROMPT` | 小哲电商智能客服 Agent 完整人设（职责/语气/安全边界/格式四段） |
...
2. **LLM 出站日志**：每次模型调用记录 `model / durMs / attempt`（如 `LLM出站 model=siliconflow-small durMs=7405 ok=true attempt=1/2`），93s 静默挂起这类「无日志空白」从此不可能再发生——空白即线索；
...
- **人设一致**：每轮 system prompt 首段均为小哲电商客服人设。
...
| **RAG 语料缺口** | 种子语料 6 条，无开票场景；开票类问题检索不到政策 | 补录开票知识片段 |
