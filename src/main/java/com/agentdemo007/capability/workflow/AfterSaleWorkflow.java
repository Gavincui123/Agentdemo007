 ...
 * <ul>
 *   <li>return/refund 流各注入一个参数化 {@link AfterSaleWorkflowGraph}（D1：政策域+资格裁决器+工单提交器参数化），
 *       本步按 {@code rp.intent()} 选图（"return_request"→退货图，否则→退款图）；</li>
 *   <li>单测用 scripted lambda 返固定终态，聚焦 {@link WorkflowExecutionStep} 的收口逻辑
 *       （图内部 5 节点 DAG 由 {@link AfterSaleWorkflowGraphTest} 覆盖，不在此重跑）。</li>
 * </ul>
 *
 * <p>语义镜像既有 seam（{@link WorkflowApprovalSubmitter}）：图<b>不直接产 {@code PipelineResult}</b>，
 * 写 context 强类型字段（presetReply），终态由顶层 {@code PipelineExecutor} 收口（④统一收口——换子图不换出口）。
 */
@FunctionalInterface
public interface AfterSaleWorkflow {

    /** 驱动售后固定工作流子图，返回终态（Rejected/Pending 提交制两态）。异常由实现包 {@link IllegalStateException}。 */
